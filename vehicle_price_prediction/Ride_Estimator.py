...
final_fi = load_feature_importance("Importance_feature.xlsx")

# Sidebar setup
image_sidebar = Image.open('Side_bar.jpg')
...
# Top banner
image_banner = Image.open('Banner.jpg')
st.image(image_banner, use_column_width=True)

# Centered title
st.markdown("<h2 style='text-align: center;color: red;'>RIDE ESTIMATOR - Vehicle Price Prediction App</h2>", unsafe_allow_html=True)
...
with left_col:
    ...
    fig = px.bar(
        ...
        color_discrete_sequence=['#FF0000']  # Custom bar color
    )
    fig.update_layout(
